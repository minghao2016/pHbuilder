...
class sim:
    ...
    def generate_phdata(self, pH, lambdaM, nstOut, barrierE):
        ...
        # TAKEN FROM BENCHMARK EXAMPLE, not calibrated.
        LYS_pKa   = 4.25
        LYS_dvdl  = [24.685, -577.05, 137.39, -172.69]
        LYS_atoms = [' CE ', ' HE1', ' HE2', ' NZ ', ' HZ1', ' HZ2', ' HZ3']
        LYS_qqA   = [0.21, 0.05, 0.05, -0.3, 0.33, 0.33, 0.33]
        LYS_qqB   = [0.13, 0.075, 0.075, -0.96, 0.34, 0.34, 0.00]

        # TAKEN FROM BENCHMARK EXAMPLE, not calibrated.
        CYS_pKa   = 4.25
        CYS_dvdl  = [24.685, -577.05, 137.39, -172.69]
        CYS_atoms = [' CB ', ' HB1', ' HB2', ' SG ']
        CYS_qqA   = [-0.11, 0.09, 0.09, -0.07]
        CYS_qqB   = [-0.1 , 0.09, 0.09, -0.08]
        ...
        for residue in self.d_residues:
            ...
            if (residue.d_resname == 'CYS'):
                acidicResidueNameList.append('CYS')
                acidicResidueNumberList.append(residue.d_resid)

            if (residue.d_resname == 'LYS'):
                acidicResidueNameList.append('LYS')
                acidicResidueNumberList.append(residue.d_resid)
        ...
        if ('CYS' in acidicResidueNameList):
            acidicResidueTypeList.append('CYS')

        if ('LYS' in acidicResidueNameList):
            acidicResidueTypeList.append('LYS')
        ...
        for idx in range(0, len(acidicResidueTypeList)):
            ...
            if (acidicResidueTypeList[idx] == 'CYS'):
                writeBlock(idx + 1, 'CYS', CYS_dvdl, CYS_pKa, barrierE, CYS_qqA, CYS_qqB)

            if (acidicResidueTypeList[idx] == 'LYS'):
                writeBlock(idx + 1, 'LYS', LYS_dvdl, LYS_pKa, barrierE, LYS_qqA, LYS_qqB)
        ...
        for residue in self.d_residues:         # loop through all residues
            ...
            for atom in residue.d_atoms:        # for each residue, loop through the atoms
                ...
                elif (residue.d_resname == 'CYS' and atom in CYS_atoms):
                    indexList.append(atomCount)

                elif (residue.d_resname == 'LYS' and atom in LYS_atoms):
                    indexList.append(atomCount)
